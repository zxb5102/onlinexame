...
$(function () {
    $('#te').tree({
        url:'../../json/sys/category_manage/paper_manage/tree_data1.json',
        lines:true,
        method:'get',
        animate:true,
        dnd:true,
        ...
        onAfterEdit:function (node) {
            if(isRepeat(node.text)){
                ...
                var text = node.text;
                $.ajax({
                    url:'../../json/sys/category_manage/paper_manage/nodeEdit.json',
                    method:'get',
                    dataType:"json",
...
function append(pareFolder,isFolder){
    ...
    $.ajax({
        url:'../../json/sys/category_manage/paper_manage/nodeAppend.json',
...
function delFile(node) {
    ...
    if( num > 0){
        ...
        $.ajax({
            url:"../../json/sys/category_manage/paper_manage/nodeDel.json",
            dataType:"json",
            data:{id:node.id},
            success:function (data) {
                if(data.flag == 'true'){
