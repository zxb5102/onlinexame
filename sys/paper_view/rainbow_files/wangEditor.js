...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            var img;
            if (resultText.state != 'SUCCESS') {
                // 提示错误
                E.warn('上传失败：' + resultText);
                alert(resultText.state);
            } else {
                E.log('上传成功，即将插入编辑区域，结果为：' + resultText);
                img = document.createElement('img');
                img.onload = function () {
                    // 将结果插入编辑器
                    var html = '<img src="' + resultText.url + '" alt="' + originalName + '" style="max-width:100%;"/>';
                    editor.command(null, 'insertHtml', html);

                    E.log('已插入图片，地址 ' + resultText.url);
                    img = null;
                };
